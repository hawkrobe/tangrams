...
full_messages_to_delete <- messagesFiltered %>%
  ...
  mutate(old = str_replace_all(contents, corrections),
         new = "") %>%
  select(old, new) %>%
  distinct() %>%
  spread(old, new)
...
partial_messages_to_swap <- messagesFiltered %>%
  ...
  mutate(old = str_replace_all(contents.x, corrections), 
         new = contents.y) %>%
  select(old, new) %>%
  spread(old, new) 
...
metacorrector = read_json('../data/tangrams_sequential/message/meta-cleaning.json')
...
sequentialCombined <- sequentialCombined.raw %>%
  ...
  mutate(contents = str_replace_all(contents, unlist(metacorrector, use.names = T))) %>%
  ...
  left_join(singleUtts %>% 
    group_by(repetitionNum, gameid) %>%
    summarize(repetitionScore = sum(correct))) %>%
  filter(numRawWords > 1) %>% # filter out empty messages
