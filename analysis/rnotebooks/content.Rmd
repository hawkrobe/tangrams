...
distinctiveness_d %>%
  ...
  tidyboot_mean(finalRoundMatch,nboot = 1000) %>%
...
true_lmer.within.out <- true_lmer.within %>%
  lmer(sim ~ poly(rep1, 2) + 
         (1 + poly(rep1, 1) | gameID) + 
         (1 + poly(rep1, 1) | target), 
