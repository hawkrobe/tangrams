...
feedback.toplot <- feedback %>%
  group_by(comparison, prevcorrect) %>%
  tidyboot_mean(column = reduction, na.rm =T, nboot = 1000) 

feedback.toplot %>%
  ungroup() %>%
  mutate(comparison = ifelse(comparison == 'rep', 'next repetition', 'next trial')) %>%
  ggplot(aes(x = prevcorrect, y = empirical_stat, fill = factor(comparison))) +
...
ggsave('../../writing/cognitive_science_format/figs/responseToFeedback_withtrial.pdf', 
