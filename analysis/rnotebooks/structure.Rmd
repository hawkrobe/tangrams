...
feedback <- listenerMsgs %>% 
  ...
  mutate(comparisonNumeric = ifelse(comparison == "rep", 1, 0)) 
...
```{r}
library(optimx)
summary(lmer(reduction ~ 1 + prevcorrect + I(repetitionNum) 
...
summary(lmer(reduction ~ prevcorrectFactor * comparison + 
               (1 + prevcorrectFactor * comparison | gameid),
#             control=lmerControl(optimizer="bobyqa",optCtrl=list(maxfun=100000)),
             data = feedback ))
library(brms)
library(rstan)
summary(brm(reduction ~ prevcorrectFactor * comparison + 
               (1 + prevcorrectFactor * comparison | gameid),
            chains = 1,
            iter = 5000,
             data = feedback ))
...
```{r}
feedback %>% filter(repetitionNum == 1) %>% group_by(matcherBin) %>% summarize(m = 1 - mean(correct))

tab <- table((feedback %>% filter(repetitionNum == 1))$matcherBin, (feedback %>% filter(repetitionNum == 1))$correct)
chisq.test(tab)
dist(rbind(feedback$matcherBin, feedback$correct), method = "binary")
