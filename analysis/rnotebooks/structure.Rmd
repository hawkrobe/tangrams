...
ggsave('../writing/tangrams/figs/listenerResponses.pdf', 
       height = 8, width = 8, units = 'cm', useDingbats = F)
```
...
feedback <- listenerMsgs %>% 
  group_by(gameid) %>% arrange(gameid, trialNum) %>%  mutate(
    trial_prevcorrect = lag(correct),
    trial_reduction = (numRawWords - lag(numRawWords))) %>%
  group_by(gameid, intendedName) %>% arrange(gameid, repetitionNum) %>% mutate(
    rep_prevcorrect = lag(correct), 
    rep_reduction = (numRawWords - lag(numRawWords))) %>% 
  ...
  mutate(prevcorrect = as.factor(ifelse(prevcorrect == 1, 'correct', 'incorrect'))) %>%
  mutate(prevcorrect = as.factor(prevcorrect))

contrasts(feedback$prevcorrect) <- contr.sum(2)
summary(lmer(reduction ~ prevcorrect * comparison +  repetitionNum + (1 + prevcorrect | gameid), 
             data = feedback %>% mutate(repetitionNum = scale(repetitionNum, scale = F))))
```

```{r}
feedback.toplot <- feedback %>%
  group_by(repetitionNum, comparison, prevcorrect) %>%
  tidyboot_mean(na.rm =T) 
...
ggsave('../writing/tangrams/figs/responseToFeedback.pdf', 
...
tagged_df %>%
   ...
   ggplot(aes(x = repetitionNum, y = empirical_stat)) + #, color = taskVersion
    ...
    theme_few() +
    theme(aspect.ratio = 1)
ggsave('../writing/tangrams/figs/wordReduction.pdf',
        height = 8, width = 10, units = 'cm', useDingbats = F)
```
...
tagged_df %>%
  ...
  mutate(OTHER = (1 - nouns - verbs - dets - #pronouns - conjunctions - adverbs
                      preps - adjectives)) %>%
  gather(POS, prop, nouns:OTHER) %>%
  mutate(POS = factor(POS, levels = c('nouns',  'verbs', 'adjectives', #'adverbs',
                              'preps', 'dets', 'OTHER'))) %>%
  ...
  ggplot(aes(x = repetitionNum, y = prop, fill = POS)) +
    ...
    ylab('% words')
ggsave('../writing/tangrams/figs/posChange.pdf',
        height = 8, width = 16, units = 'cm', useDingbats = F)
...
ggsave('../writing/tangrams/figs/posReduction.pdf',
        height = 8, width = 16, units = 'cm', useDingbats = F)
```
...
true <- 4.362526534753067
permuted <- read_delim('../outputs/permuted_dependency_distribution.txt', ',', col_names = c('vals'))
cat('95% CI:', sort(permuted$vals)[2], ',', sort(permuted$vals)[98])
```
