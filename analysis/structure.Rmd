...
feedback <- listenerMsgs %>% 
  ...
  mutate(prevRoundCorrect = lag(correct), 
         reduction = (numRawWords - lag(numRawWords))/lag(numRawWords )) %>% 
  filter(!is.na(prevRoundCorrect)) %>%
  mutate(prevRoundCorrect = as.factor(ifelse(prevRoundCorrect == 1, 'correct', 'incorrect')))

feedback$prevRoundCorrect <- as.factor(feedback$prevRoundCorrect)
contrasts(feedback$prevRoundCorrect) <- contr.sum(2)
summary(lmer(reduction ~ prevRoundCorrect + repetitionNum + (1 + prevRoundCorrect | gameid), 
             data = feedback %>% mutate(repetitionNum = scale(repetitionNum, scale = F))))
```

```{r}
feedback.toplot <- feedback %>%
  group_by(repetitionNum, prevRoundCorrect) %>%
  tidyboot_mean(reduction, na.rm =T) 

feedback.toplot %>%
  ggplot(aes(x = repetitionNum, y = empirical_stat, color = factor(prevRoundCorrect))) +
    geom_point() +
...
true <- 4.229125892678514
permuted <- read_delim('./outputs/permuted_distribution.txt', ',', col_names = c('vals'))
