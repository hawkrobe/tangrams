...
distinctiveness_d %>% mutate(isZero = tf.idf == 0) %>% group_by(finalRoundMatch, isZero) %>% tally()
summary(glmer(finalRoundMatch ~ tf.idf + (1 + tf.idf | intendedName) + (1 + tf.idf | gameid), 
...
distinctiveness_d %>%
  filter(tf.idf != 0) %>%
  mutate(bin = .6*floor(tf.idf / .6) + .3) %>%
  group_by(bin) %>%
  tidyboot_mean(finalRoundMatch) %>%
  rename(sample_size = n) %>%
  ggplot(aes(x = bin, y =empirical_stat)) +
    geom_point(aes(size = sample_size), stat = 'identity') +
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0.1) +
    ylim(0, .25) +
    ylab('% final convention') +
    xlab('distinctiveness on initial repetition (binned)') +
    xlim(0,3.5) +
    theme_few() +
