...
```{r, cache = T, warning=F}
tagged_df <- read_csv('./outputs/posTaggedtangramsSequential_collapsed.csv') %>%
  #filter(role == 'director') %>%
  select(-text, -contents, -posCounts, -pos) %>%
  left_join(read_csv('./outputs/constituency_tags.csv') %>%
...
tagged_df %>%
  #filter(taskVersion == 'cued') %>%
  select(gameid, repetitionNum, SBAR, CC, PP) %>% #, taskVersion
...
ggsave('../writing/tangrams/figs/syntacticReduction.pdf', width = 8, height = 3)
```
...
ggsave('../writing/tangrams/figs/tsne_embeddings.pdf', width = 15, height = 15)
```
