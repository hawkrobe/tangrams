...
empirical_baselines.drift <- combine_empirical_and_baselines(M_mat, F_mat, 'drift', 10) %>%
    ...
    bind_rows(compute_across_similarity(M_mat, F_mat, 'baseline', 'cosine') %>%
                filter(repetition == 1) %>% select(-n, -mean, -repetition) %>%
                mutate(repdiff = '1->1')) 

empirical_baselines.drift %>%
  ggplot(aes(x = repdiff, y = empirical_stat, color = sample_id, linetype = sample_id, group = sample_id)) +
...
true_lmer.within.out <- lmer(sim ~ poly(rep1,2) + (1 + poly(rep1,1) | gameID) + (1+ poly(rep1,1)| target), data = true_lmer.within)
...
combined.within %>%
  ggplot(aes(x = repdiff, y = empirical_stat, color = sample_id, linetype = sample_id, group = sample_id)) +
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0, data = combined.within %>% filter(sample_id == 'empirical')) +
    geom_line()  +
    geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper), alpha = 0.5, color = NA, data = combined.within %>% filter(sample_id == 'baseline')) +
...
true_lmer.across.out <- lmer(sim ~ poly(repetition, 2) + (1 | gamepair) + (1 | target), data = true_lmer.across)
...
ggplot(combined.across, aes(x = IV, y = empirical_stat, 
               fill = sample_id, color = sample_id, group = sample_id)) +
    geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper), width = 0, data = combined.across %>% filter(sample_id == 'empirical')) +
    geom_line()  +
    geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper), alpha = 0.5, color = NA, data = combined.across %>% filter(sample_id == 'baseline')) +
