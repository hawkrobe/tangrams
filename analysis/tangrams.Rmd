...
unconstrainedData.raw <- unconstrainedMsgs %>% left_join(unconstrainedSubjInfo, by = c('gameid', 'role'))
```

Set exclusion criteria

```{r}
nonNativeSpeakerIDs <- unique((unconstrainedTangramSubjInfo %>% filter(nativeEnglish != "yes"))$gameid)
...
print(paste0('excluding ', length(badGames), ' of ', length(unique(unconstrainedData.raw$gameid)), ' games that were either incomplete or had non-native english speakers'))
```
...
```{r}
tangramMsgs = read_csv("../data/tangrams_sequential/message/sequential_message_raw.csv") %>%
  rename(msgTime = time, role = sender)
...
rawTangramCombined <- tangramMsgs %>% 
  left_join(tangramSubjInfo, by = c('gameid', 'role')) %>% 
  left_join(tangramClicks, by = c('gameid', 'roundNum', 'occurrenceNum'))
```
...
duplicate_gameids <- c('0210-5d5ad8b6-7c94-4e2a-a3cb-760d2f613953', '9087-3c0f9d65-0427-406f-9251-94da5d2dee54', '7261-07035df8-84ee-4381-b72c-a04234c8f5ed')
...
sketchy_gameids <- c('9116-d8b8c5c1-f549-48bf-a362-ed159302a106', # This was super distracted and kept complaining and talking about how they'd skip through rounds...
                     '7229-0a7402b0-329e-4b26-bc3a-ae26a388a6cc' # This one was having a lot of fun with wordplay and stuff...
                     )

```

Create filtered version of messages data (for potential hand-cleaning)

```{r}
messagesFiltered <- tangramMsgs %>%
  filter(!(gameid %in% badGames)) %>%
...
```{r}
library(ggrepel)
single_utt.toplot <- tangramSingleUtt %>%
  group_by(occurrenceNum, intendedObj) %>%
  summarize(propCorrect = sum(correct)/length(correct)) %>%
  mutate(intendedObjLabel = ifelse(intendedObj %in% c('C', 'B', 'G', 'E'), intendedObj, ''))

ggplot(single_utt.toplot, aes(x = occurrenceNum, y = propCorrect, group = intendedObj, label = intendedObjLabel)) +
    geom_line(aes(color = (intendedObjLabel == "")), alpha = 1,  size = 1.5) +
    geom_text_repel(data = subset(single_utt.toplot, occurrenceNum == min(occurrenceNum)),
