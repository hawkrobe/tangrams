...
```{r}
compute_within_drift(M_mat, F_mat, 'empirical', method = 'cosine', nboot = 1000) %>%
  ...
  ggplot(aes(x = repdiff, y = empirical_stat, 
...
ggsave('../writing/tangrams/figs/drift.pdf', height = 10, width = 7, units = 'cm', useDingbats = F)
```
...
compute_within_convergence(M_mat, F_mat, 'empirical', method = 'cosine', nboot = 1000) %>%
  ggplot(aes(x = `rep diff`, y = empirical_stat, fill = sample_id, color = sample_id, group = sample_id)) +
      ...
      theme(aspect.ratio = 1, legend.position = c(0.5, 0.8), text = element_text(size=18), 
            element_line(size=1), element_rect(size=2, color="#00000"))
ggsave('../writing/tangrams/figs/stability.pdf', height = 10, width = 7, units = 'cm', useDingbats = F)
...
compute_across_similarity(M_mat, F_mat, 'raw_cor_nonorm', method = 'cosine', nboot = 100) %>%
  ggplot(aes(x = repetition, y = empirical_stat, 
    ...
    theme(aspect.ratio = 1, legend.position = c(0.5, 0.8), text = element_text(size=18), 
          element_line(size=1), element_rect(size=2, color="#00000"))
ggsave('../writing/tangrams/figs/divergence.pdf', height = 10, width = 7, units = 'cm', useDingbats = F)
```
