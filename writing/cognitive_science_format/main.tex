...
Next, we examined the extent to which directors were sensitive to the ground-truth feedback that was provided at the end of each trial about which tangram the matcher actually \emph{selected}.
...
If uncertain speakers initially produce an utterance from a broad distribution of acceptable labels, and update their distribution on subsequent repetitions, different pairs may end up in different equilibria due to randomness in sampling from a more or less shared initial distribution. This is the mechanism proposed in recent probabilistic models of convention formation \citep{smith_learning_2013,hawkins_convention-formation_2017,Brochagen17}, which have captured several of these properties in simulations.
...
These two possibilities -- initial idiosyncratic preferences or initial uncertainty and breadth -- are not mutually exclusive.
...
Although our analyses go beyond previous work by using new vector-space semantic models, they still have some limitations based on these models.
...
Further, though our dyads are likely diverse in many ways relative to US national population \citep{levay2016demographic} they are in no way representative of either the US population or any broader population.
Thus, further cross-cultural work examining the validity of our conclusions across populations would be a valuable contribution for future work.
