...
Human language use is remarkably flexible. % and adaptive to the needs of the current situation. 
...
Slang, technical lingo, nicknames, inside jokes, metaphors, and other neologisms may coalesce over timescales as short as a single conversation. % all operate in a regime of uncertainty where the same word or phrase may \emph{a priori} mean something different (or nothing at all) to different partners in different contexts. (cite, cite).
...
% a lifetime of learning the conventions of our language community provides a crucial backbone for understanding each other \cite{Lewis69_Convention}, we frequently find ourselves in situations where our existing lexicon falls short.
%For one, no two speakers share exactly the same lexicon \cite{, Clark98_CommunalLexicons}. 
...
For example, \dots \todo[inline]{Mention here}
...
% in this literature. %on repeated reference games has 
