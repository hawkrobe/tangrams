...
To operationalize these constructs, we introduce a continuous measure of similarity based on distances computed between continuous vector space embeddings of referring expressions (see Appendix A for an alternative approach based on discrete word distributions). 
...
To gain an intuition about the changes uncovered by these analyses of utterance embeddings, we first visualize the trajectories taken by each pair of participants when referring to a particular example tangram, annotating utterances in several parts of the space.
...
\section*{Appendix A: Measuring convergence and divergence using discrete word distributions}
