...
Finally, we computed the mean dependency lengths between all such pairs of dropped word on each given trial, and took the mean across all trials. 
...
We found a mean empirical dependency length of 4.22, which lay outside the both the random null distribution ($95\%$CI: $[4.38, 4.44]$) and the function word null distribution($95\%$ CI: $[]$), indicating a small but reliable effect of syntactic clustering among the words that were dropped on each round.
...
Similarly how the \emph{ad hoc} meanings formed in one novel context generalize to other contexts with the same partner.
...
\begin{figure}[b!]
\centering
\includegraphics[scale=.6]{syntactic_reduction_baselines.pdf}
