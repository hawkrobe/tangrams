...
If averaging together greater numbers of word vectors generically washes out differences and causes the resulting utterance vectors to be more similar one another, then the decrease in semantic similarity could be explained by the decrease in utterance \emph{length} (see Section 4.2) rather than divergence in content.
We tested this null hypothesis using a further permutation test. 
Specifically, we reasoned that if the effect is in fact driven by length, then the similarity measured across interactions should be invariant to re-sampling utterance content---the individual words that will be averaged together---within interactions.
We thus scrambled the words used by a participant across all twelve tangrams at each repetition, destroying any tangram-specific shared content across games, but preserving utterance length.
By repeating this procedure 100 times, we found that the true mean similarity across games was higher than predicted under the null distribution for all six repetitions, $p < 0.01$, suggesting that our semantic similarity measure is picking up on something more than pure utterance length.
...
One possibility is that there is already substantial baseline similarity in how a single speaker refers to different tangrams.
Do utterances used by the same speaker to refer to different tangrams \emph{within} an interaction diverge to the same degree as utterances used by different speakers to refer to the same tangram?
