...
We begin with the observation that the mean number of words used by directors decreases over time (see Fig. \ref{fig:feedback}A)\footnote{A similar reduction curve was found in our  ``free matching'' pilot version of the task, though it required more words overall. Participants needed to additionally mention which tangram they were referring to (i.e. ``number 3 is the \dots``)}.
...
While we automatically supplied minimal feedback about the matcher's response and the true target on each trial, we predicted that additional feedback provided through backchannel replies should be highest on the first repetition and drop off once meanings are agreed upon. 
...
In aggregate, 75\% of matchers responded with at least one message in the first repetition block, but only 4\% sent a message in the last block.
...
The sensitivity to feedback we observed on the subsequent repetition block is not present on the subsequent trial: speakers are equally likely to use more or less words after a correct response, and actually use fewer words after an \emph{incorrect} response due to a regression to the mean: more words than average are used for harder tangrams. 
This pattern of results is consistent with sensitivity to \emph{tangram-specific} evidence of the matcher's understanding.
...
Having established the conditions under which speakers are willing to shorten their utterances, we turn to structure in the shortening process.
