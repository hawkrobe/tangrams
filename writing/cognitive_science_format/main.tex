...
We scrambled utterances across different pairs of participants, preserving repetition order, and re-ran our mixed-effects model to obtain a null distribution representing the decrease in semantic distance from a \emph{random} speaker's utterance on the first repetition.
...
\ndg{the way this is phrased i feel like your trying to sell me on a result that isn't really there. the whole population seems to become more diverse in about the same way as within-pair (with that little analysis artifact at 1,2), which is consistent with all pairs having a similar initial distribution.}

\ndg{actually, i'm not sure that the analysis in the subsection adds anything? could delete and just keep the next two, which are clearer?}
