...
Another key computational signature of local conventions is their stability, or \emph{stickiness}, across changing contexts. Once a precedent has been established with a particular partner, the pressure to maintain it apparently even trumps the usual Gricean pressures of informativity. For example, if a pair of participants spend several round of a repeated reference game referring to a particular shoe in the context of other shoes, they might converge on an appropriately specific subordinate term like \emph{loafer}. Rather than swapping out the partner like the studies in the previous section, the experimenter can swap out the context, replacing all but the target shoe with objects from other categories. Remarkably, on the first round in the new context, participants continued to use the now-overinformative label (e.g. \emph{loafer}) 52\% of the time  \citeA{BrennanClark96_ConceptualPactsConversation}.

This can be understood in our model as a consequence of the path-dependence of learning. The initial context affects the initial terms produced (via RSA): several rounds of reinforcing the label \emph{loafer} when referring to a particular shoe reduces the uncertainty of that lexical entry to nearly zero (even if the lexical prior assigned very little uncertainty to begin with). When the context changes, then \dots

Recent and frequency effects further reinforce learning. 

Path-dependence: more variability across pairs than within pairs (see also Hawkins et al)
