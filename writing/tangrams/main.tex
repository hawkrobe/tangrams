...
In this paper, we explore the possibility that agents solve this problem by creating new \emph{ad hoc} conventions on the fly.
Building on the collaborative model proposed by Clark \cite{}, we propose an \emph{inferential} model 
...
We examine these first two predictions using two different signatures: one based on the discrete word count distribution and the other based on vector embeddings of semantic meaning.
...
\subsubsection{Using semantic embeddings}
